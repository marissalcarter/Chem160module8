...
while Temp<50.:
    ...
    spins=np.full((n,n),1) ## Add one line from lecture notes here
    ...
    for trial in range(1,(ntrials+nequil+1)):
        ...
        deltaE=2*spins[i][j]*(neighbors(spins,i,j,zone).sum()-spins[i][j]) ## Add one line from lecture notes here
        ...
        if trial == nequil:
            ...
            for i in range(n):
                for j in range(n):
                    energy-=spins[i][j]*(neighbors(spins,i,j,zone).sum()-spins[i][j]) ## Add one line from lecture notes here
            energy/=n2
